...
class Container {
    ...
    public function loadServices(string $namespace, ?\Closure $callback = null): void
    {
        ...
        foreach($files as $file) {
            ...
            if ($constructor != null) { // null oznacza brak zdefiniowanego konstruktora
                ...
                $this->addService($serviceName, function () use ($serviceName, $serviceParameters) {
                    foreach ($serviceParameters as &$serviceParameter) { // "odpalenie" zale¿no¶ci
                        if ($serviceParameter instanceof \Closure) {
                            $serviceParameter = $serviceParameter();
                        }
                    }

                    return new $serviceName(...$serviceParameters); // ... konwertuje tablice na liste argumentów
                });
